...
public class SignExcOps implements Operations<SignExc, TTExc> {
    ...
    private final static SignExc[] REMOVE_ZERO = {
        /*					NONE_A	NEG		ZERO	POS		ERR_A	NON_POS		NON_ZERO	NON_NEG		Z			ANY_A */
        					NONE_A,	NEG,	NONE_A,	POS,	ERR_A,	NEG,		NON_ZERO,	POS,		NON_ZERO,	ANY_A 
    };
	...
	@Override
	public SignExc removeZero(SignExc a) {
		return REMOVE_ZERO[a.ordinal()];
	}

	@Override
	public boolean possiblyZero(SignExc a) {
		return (a == ZERO || a == NON_POS || a == NON_NEG || a == Z || a == ANY_A);
	}
	
	@Override
	public boolean isDefZero(SignExc a) {
		return a == ZERO;
	}

	@Override
	public SignExc[] allAValues() {
		//return SignExc.values();
		return new SignExc[] {NEG, ZERO, POS};
	}
}
